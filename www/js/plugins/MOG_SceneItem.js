...
Scene_Item.prototype.loadBitmaps = function () {
	...
	for (var i = 0; i < 4; i++) {
		...
		this._dis_comImg[i] = ImageManager.loadMenusitem("dis_Com_" + i);
	};
};
...
Scene_Item.prototype.createButtons = function () {
	...
	this._dis_buttons = [];
	this._buttonsAni = [];
	this._dis_buttonsAni = [];
	for (var j = 0; j < 4; j++) {
		this._dis_buttonsAni[j] = 0;
		this._dis_buttons[j] = new Sprite(this._dis_comImg[j]);
		this._dis_buttons[j].anchor.x = 0.5;
		this._dis_buttons[j].anchor.y = 0.5;
		this._dis_buttons[j].opacity = 255;
		this._dis_xSpace = 42;
		this._dis_ySpace = 27;
		if (j === 0) {
			this._dis_buttons[j].x = this._dis_xSpace + 54;
			this._dis_buttons[j].y = this._dis_ySpace + 129;
		} else if (j === 1) {
			this._dis_buttons[j].x = this._dis_xSpace + 196;
			this._dis_buttons[j].y = this._dis_ySpace + 129;
		} else if (j === 2) {
			this._dis_buttons[j].x = this._dis_xSpace + 336;
			this._dis_buttons[j].y = this._dis_ySpace + 129;
		} else if (j === 3) {
			this._dis_buttons[j].x = this._dis_xSpace + 476;
			this._dis_buttons[j].y = this._dis_ySpace + 129;
		};
		this._field.addChild(this._dis_buttons[j]);
	}
	...
	for (var i = 0; i < 4; i++) {
		...
		this.y_space = 27;
		if (i === 0) {
			this._buttons[i].x = this.x_space + Moghunter.scItem_Com1_X;
			this._buttons[i].y = this.y_space + Moghunter.scItem_Com1_Y;
		} else if (i === 1) {
			this._buttons[i].x = this.x_space + Moghunter.scItem_Com2_X;
			this._buttons[i].y = this.y_space + Moghunter.scItem_Com2_Y;
		} else if (i === 2) {
			this._buttons[i].x = this.x_space + Moghunter.scItem_Com3_X;
			this._buttons[i].y = this.y_space + Moghunter.scItem_Com3_Y;
		} else if (i === 3) {
			this._buttons[i].x = this.x_space + Moghunter.scItem_Com4_X;
			this._buttons[i].y = this.y_space + Moghunter.scItem_Com4_Y;
		};
...
Scene_Item.prototype.updateButtons = function () {
	...
	for (var j = 0; j < this._dis_buttons.length; j++) {
		if (this._categoryWindow._index === j && !this._actorWindow.active) {
			if (this._dis_buttonsAni[j] === 1) {
				this._dis_buttons[j].scale.x += 0.015;
				if (this._dis_buttons[j].scale.x >= 1.15) {
					this._dis_buttons[j].scale.x = 1.15;
					this._dis_buttonsAni[j] = 1;
				};
			} else {
				this._dis_buttons[j].scale.x -= 0.015;
				if (this._dis_buttons[j].scale.x <= 1.15) {
			...
			this._dis_buttons[j].opacity += 200;
		} else {
			this._dis_buttonsAni[j] = 1
			if (this._dis_buttons[j].scale.x >= 1.00) {
				this._dis_buttons[j].scale.x -= 0.010;
				if (this._dis_buttons[j].scale.x <= 1.00) {
					this._dis_buttons[j].scale.x = 1.00;
				};
			};
			if (this._dis_buttons[j].opacity > 255) {
				this._dis_buttons[j].opacity -= 15;
			};
		};

		this._dis_buttons[j].scale.y = this._dis_buttons[j].scale.x;
	}
};
...
Scene_Item.prototype.updateItemLayout = function () {
	this._layoutItem.x = Moghunter.scItem_WlayoutX;
	this._layoutItem.y = Moghunter.scItem_WlayoutY;
...
Scene_Item.prototype.createItemWindow = function () {
	...
	var wx = Moghunter.scItem_ItemWindowX;
