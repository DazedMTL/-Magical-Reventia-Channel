...
ImageManager.loadMenuBg = function(filename) {
	return this.loadBitmap('img/menus/', filename, 0 ,true);
}

ImageManager.loadMenuCg = function(filename) {
	return this.loadBitmap('img/menus/CG/', filename, 0 ,true);
}
...
Scene_Menu.prototype.loadBitmapsMain = function() {
	...
	this._goldImg = ImageManager.loadMenusMain("numbersOutline");
};
     ...
     for (var i = 0; i < this._commands.length; i++) {
		  if (this.isComEnabled(i)) {
			   ...
			   if (this._comzoom[i] === 0 && !this._statusWindow.active) {
				   this._commands[i].scale.x += 0.005;
				   if (this._commands[i].scale.x >= 1.20) {
					   this._commands[i].scale.x = 1.20;
				       this._comzoom[i] = 1;
				   };
			   } else {
				   this._commands[i].scale.x -= 0.005;
...
MCharStatus.prototype.loadBitmaps = function() {
   this._numberImg = ImageManager.loadMenusMain("numbersOutline");
   this._numberImg2 = ImageManager.loadMenusMain("numbersOutline");
   this._numberImg3 = ImageManager.loadMenusMain("numbersOutline");
   this._machoNumberImg = ImageManager.loadMenusMain("machoNumber");
   this._leaderNumberImg = ImageManager.loadMenusMain("leaderNumber");
   this._succuNumberImg = ImageManager.loadMenusMain("succuNumber");
   this._slimeNumberImg = ImageManager.loadMenusMain("slimeNumber");

//    this._HPMeterImg = ImageManager.loadMenusMain("HPMeter");
//    this._MPMeterImg = ImageManager.loadMenusMain("MPMeter");
//    this._succuMeterImg = ImageManager.loadMenusMain("succuMeter");
//    this._machoMeterImg = ImageManager.loadMenusMain("machoMeter");
//    this._LeaderMeterImg = ImageManager.loadMenusMain("LeaderMeter");
//    this._slimeMeterImg = ImageManager.loadMenusMain("slimeMeter");
...
MCharStatus.prototype.createSprites = function() {
	...
	// this.createHPMeter();
	// this.createMPMeter();
	// this.createSuccuMeter();
	// this.createMachoMeter();
	// this.createLeaderMeter();
	// this.createSlimeMeter();
	this.createHPNumber();
	// this.createMPNumber();
	...
	this.createStates();
	// this.createName();
	this.createMachoNumber();
	this.createLeaderNumber();
	this.createSuccuNumber();
	this.createSlimeNumber();
	this.kindCountSubsNumber();
	this.badCountSubsNumber();
	this.sexyLevelNumber();
	this.moraleNumber();
	this.ruisekiNumber();
	this.maxsexyLevelNumber();
};	
...
MCharStatus.prototype.createHPMeter = function() {
	...
	// this.updateMeter(this._hpMeter,this._hpMeterData,$gameVariables.value(7),$gameVariables.value(9));
...
// * create macho Meter
//==============================
// MCharStatus.prototype.createMachoMeter = function() {
// 	this._machoMeter = new Sprite(this._machoMeterImg);
// 	this._machoMeterData = [250,this._machoMeterImg.height,0,0];
// 	this._machoMeterData[2] = 250 * 2;
// 	this._machoMeterData[3] = Math.randomInt(this._machoMeterData[2])
// 	this._machoMeter.x = 55;
// 	this._machoMeter.y = 371;
// 	this.updateMeter(this._machoMeter,this._machoMeterData,this._actor.mp,this._actor.mmp);
// 	this.addChild(this._machoMeter);
// };  
...
// * create leader Meter
//==============================
MCharStatus.prototype.createLeaderMeter = function() {
	this._LeaderMeter = new Sprite(this._LeaderMeterImg);
	this._LeaderMeterData = [this._LeaderMeterImg.width / 3,this._LeaderMeterImg.height,0,0];
	this._LeaderMeterData[2] = this._LeaderMeterData[0] * 2;
	this._LeaderMeterData[3] = Math.randomInt(this._LeaderMeterData[2])
	this._LeaderMeter.x = this._layout.x + Moghunter.scMenu_MPMeterX;
	this._LeaderMeter.y = this._layout.y + Moghunter.scMenu_MPMeterY + 50;
	this.updateMeter(this._LeaderMeter,this._LeaderMeterData,this._actor.mp,this._actor.mmp);
	this.addChild(this._LeaderMeter);
};  

//==============================
// * create SUCCU Meter
//==============================
MCharStatus.prototype.createSuccuMeter = function() {
	this._succuMeter = new Sprite(this._succuMeterImg);
	this._succuMeterData = [this._succuMeterImg.width / 3,this._succuMeterImg.height,0,0];
	this._succuMeterData[2] = this._succuMeterData[0] * 2;
	this._succuMeterData[3] = Math.randomInt(this._succuMeterData[2])
	this._succuMeter.x = this._layout.x + Moghunter.scMenu_MPMeterX;
	this._succuMeter.y = this._layout.y + Moghunter.scMenu_MPMeterY + 100;
	this.updateMeter(this._succuMeter,this._succuMeterData,$gameVariables.value(43),100);
	this.addChild(this._succuMeter);
}; 

//==============================
// * create slime Meter
//==============================
MCharStatus.prototype.createSlimeMeter = function() {
    this._slimeMeter = new Sprite(this._slimeMeterImg);
	this._slimeMeterData = [this._slimeMeterImg.width / 3,this._slimeMeterImg.height,0,0];
	this._slimeMeterData[2] = this._slimeMeterData[0] * 2;
	this._slimeMeterData[3] = Math.randomInt(this._slimeMeterData[2])
	this._slimeMeter.x = this._layout.x + Moghunter.scMenu_MPMeterX;
	this._slimeMeter.y = this._layout.y + Moghunter.scMenu_MPMeterY + 150;
	this.updateMeter(this._slimeMeter,this._slimeMeterData,this._actor.mp,this._actor.mmp);
	this.addChild(this._slimeMeter);
};  
...
MCharStatus.prototype.createHPNumber = function() {
    this._hp_number = [];
	this._NumberData = [16,this._numberImg.height]
	...
	this.refresh_number(this._hp_number,undefined,this._NumberData,x)
};	
	
//==============================
// * create HP Number2
//==============================
MCharStatus.prototype.sexyLevel = function() {
    this._sexyLevel = [];
	
	var y = 57;
	this._sexyLevelValue = $gameVariables.value(45);
	if (this._sexyLevelValue > 9) {
		var x = 731;
	} else {
		var x = 627;
	}
	for (var i = 0; i < 5; i++) {
		 this._sexyLevel[i] = new Sprite(this._numberImg);
		 this._sexyLevel[i].visible = false;
		 this._sexyLevel[i].opacity = 255;
		 this._sexyLevel[i].x = x ;
		 this._sexyLevel[i].y = y ;
		 this.addChild(this._sexyLevel[i]);		
		 console.log(i); 
	};	
	// this.refresh_number(this._sexyLevel,this._actor.mhp,this._NumberData,x)
	this.refresh_number(this._sexyLevel,this._sexyLevelValue,this._NumberData,x)
};	
MCharStatus.prototype.maxLevel = function() {
    this._sexyLevel = [];
...
MCharStatus.prototype.createLVNumber = function() {
	...
	this.refresh_number(this._lv_number,$gameVariables.value(2),this._NumberData3,x)
};	
...
MCharStatus.prototype.update = function() {
	...
	if (this._hpMeter) {this.updateMeter(this._hpMeter,this._hpMeterData,$gameVariables.value(7),$gameVariables.value(9))};
	if (this._mpMeter) {this.updateMeter(this._mpMeter,this._mpMeterData,this._actor.mp,this._actor.mmp)};
	if (this._succuMeter) {this.updateMeter(this._succuMeter,this._succuMeterData,$gameVariables.value(43),100);};
	if (this._machoMeter) {this.updateMeter(this._machoMeter,this._machoMeterData,this._actor.mp,this._actor.mmp);};
	if (this._LeaderMeter) {this.updateMeter(this._LeaderMeter,this._LeaderMeterData,this._actor.mp,this._actor.mmp);};
	if (this._slimeMeter) {this.updateMeter(this._slimeMeter,this._slimeMeterData,this._actor.mp,this._actor.mmp);};
