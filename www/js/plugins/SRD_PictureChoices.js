...
(function (_) {
	...
	Window_PictureChoiceList.prototype.updatePlacement = function () {
		this.width = (this.windowWidth() * 7) / 8;
	...
	Window_PictureChoiceList.prototype.drawItem = function (index) {
		...
		if (name.match(/\\picture\[(.*)\]/i)) {
			...
			sprite.x = (rect.x - 100) + Graphics.boxWidth / 5;
			sprite.y = rect.y + ((rect.height * 5.7) / 9);
			...
			sprite.y = rect.y + ((rect.height * 2) / 3);
			this._spriteChoices[index] = sprite;
			this.addChild(sprite);
		}
	};
	...
	Sprite_PictureChoice.prototype.update = function () {
		...
		if (this._direction) {
			if (this._direction === 1) {
				this.opacity -= (_.close * 20);
				// this._breath += _.close;
				this._breath += 0.005;

				if (this._breath > this._duration) {
					this.scale.x = this._duration;
					this.scale.y = this._duration;
				} else {
